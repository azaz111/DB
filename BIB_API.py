...
def naznach_permis(service,drive_id,email):  # Назначить манагера диску
    new_permissions = {
    'type': "user",
    'role': "organizer",
    'emailAddress': email
    }
    permission_response = service.permissions().create(fileId=drive_id, 
...
def file_all(service,rodit=None): # Список папок (на диске или в папке) выход: список фалов
    file_spis=[]
    page_token = None
    if rodit :
       filtr = f"mimeType = 'application/vnd.google-apps.file' and '{rodit}' in parents"
    else:
        filtr = f"mimeType = 'application/vnd.google-apps.file'"
    while True:
        folder_ids = service.files().list(corpora='allDrives', 
                                          includeItemsFromAllDrives=True, 
                                          supportsAllDrives=True,
